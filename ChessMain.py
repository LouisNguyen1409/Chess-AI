...
def highlightMove(screen, gs, validMoves, sqSelected):
    ...
    if gs.inCheck:
        if gs.whiteTurn:
            ...
            screen.blit(
                sq,
                (gs.whiteKingLocation[1] * SQ_SIZE, gs.whiteKingLocation[0] * SQ_SIZE),
            )
        else:
            sq.fill(p.Color("red"))
            screen.blit(
                sq,
                (gs.blackKingLocation[1] * SQ_SIZE, gs.blackKingLocation[0] * SQ_SIZE),
            )

    if len(gs.moveLog) != 0:
        sq.fill(p.Color("yellow"))
        screen.blit(
            sq, (gs.moveLog[-1].startCol * SQ_SIZE, gs.moveLog[-1].startRow * SQ_SIZE)
        )
        screen.blit(
            sq, (gs.moveLog[-1].endCol * SQ_SIZE, gs.moveLog[-1].endRow * SQ_SIZE)
        )
...
def animateMove(move, screen, board, clock):
    ...
    sqDistance = math.sqrt(
        abs(move.endRow - move.startRow) * abs(move.endRow - move.startRow)
...
def drawBoard(screen):
    ...
    for row in range(DIMENSION):
        for column in range(DIMENSION):
            ...
            p.draw.rect(
                screen, color, p.Rect(column * SQ_SIZE, row * SQ_SIZE, SQ_SIZE, SQ_SIZE)
            )
